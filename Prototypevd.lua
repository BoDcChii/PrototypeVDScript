-- [[ BoDcChii Project - v0.4.6: FINAL HYBRID INTEGRATION 🎸 ]] --
...
local function CreateTabBtn(text, page)
    ...
    btn.Text = text; btn.TextColor3 = Color3.new(0.8, 0.8, 0.8); btn.Font = Enum.Font.SourceSansBold; btn.TextSize = 11
    Instance.new("UICorner", btn).CornerRadius = UDim.new(0, 6)
    local s = Instance.new("UIStroke", btn); s.Color = Color3.fromRGB(255, 105, 180); s.Thickness = 1.5
...
CreateTabBtn("0. ABOUT", P0); CreateTabBtn("1. PLAYER ESP", P1)
CreateTabBtn("2. SURVIVAL", P2); CreateTabBtn("3. SMOOTH MAPS", P3)
...
local function CreateToggle(parent, text, callback)
    ...
    lbl.Font = Enum.Font.SourceSansBold; lbl.TextSize = 10; lbl.TextXAlignment = "Left"
...
-- About Page Content (From v0.4.6 Fixed)
local info = Instance.new("TextLabel", P0)
info.Size = UDim2.new(1,0,1,0); info.BackgroundTransparency = 1; info.TextColor3 = Color3.new(1,1,1)
info.Text = "Creator: BoDcChii\nScript Tester: Xiaoo\nVersi: v0.4.6 (FIXED)\n\nUpdate Fitur:\n- ESP Generator FIXED\n- Visual Parry Radius\n- Auto Parry Beta (9.5 Studs)\n- Fix Analog Lock (Mobile)\n- Tembus Tembok Integration"; 
info.TextSize = 11; info.Font = Enum.Font.SourceSansBold; info.TextYAlignment = "Top"; info.TextXAlignment = "Left"
...
MainBtn.TextSize = 22; MainBtn.Font = Enum.Font.SourceSansBold; Instance.new("UICorner", MainBtn).CornerRadius = UDim.new(1, 0)
local BStroke = Instance.new("UIStroke", MainBtn); BStroke.Thickness = 2.5
EnableDrag(MainBtn)

task.spawn(function()
    while task.wait() do
        local hue = tick() % 5 / 5
        BStroke.Color = Color3.fromHSV(hue, 0.6, 1)
    end
end)
...
MainBtn.MouseButton1Click:Connect(function()
    if MainFrame.Visible then
        ...
        MainFrame.Visible = true; MainFrame:TweenSize(UDim2.new(0, 400, 0, 250), "Out", "Back", 0.4, true)
    end
end)

P0.Visible = true
print("BoDcChii Project v0.4.6 Hybrid Loaded!")